...
- **Daily-collect cron now actually persists state (was failing every run).** The first real run surfaced a fatal bug — and reproducing it locally (with the runner's no-global-identity condition) uncovered a second latent one:
  ...
  2. The `.gitignore` written into `_publish` contained `state.db` — which **ignored the very file being committed**: `git add state.db` was silently skipped, so even once the commit ran it would never include `state.db`. It now ignores only the WAL sidecars (`state.db-wal`/`-shm`/`-journal`). (Masked in production because the identity error failed first.)
  - The restore step was also rewritten as a graceful shallow `git fetch` (replacing `actions/checkout@v4 ref: state`, which shows red on a cold start even with `continue-on-error`) so it stays green when the branch doesn't exist yet. The whole commit→push→restore cycle is verified locally end-to-end (cold-start orphan create, identity, state.db actually committed, second-run fast-forward, remote ends with the latest blob).
- **Daily-collect cron self-heals and never silently loses a day.** Three operations-review fixes to `.github/workflows/collect.yml`: (1) the restored `state.db` is integrity-checked on seed — a corrupt committed copy is cold-started past (logged as an error) instead of being restored every run forever, which would wedge the cron red with no recovery (the corrupt blob stays in the branch's git history for forensics); (2) the WAL checkpoint now runs *before* the integrity check so we validate the exact bytes being committed; (3) the `state`-branch push is wrapped in a re-fetch/re-apply retry loop so a concurrent writer (e.g. a manual dispatch) that advances the branch between our fetch and push no longer fails the job and drops the day's collected items. (Ops-H2 "no token ceiling" was already covered: `agent.example.yaml` sets `token_budget`, and the cron uses that config.)
- **HTTP response-size cap (pulled forward from "deferred").** The polite client now streams responses and aborts past a configurable cap (`max_response_bytes`, default 10 MiB), so a hostile/compromised source can't exhaust memory with a multi-GB or decompression-bomb body in the unattended cron. The cap applies to the *decompressed* size. `ResponseTooLargeError` is per-source-isolated like any other fetch failure.
- **Top-level CLI error redaction.** Added `main()` (the new console-script entry point) with a guard that, in production log mode, replaces an unhandled exception's traceback with just the exception class + a correlation hash — closing a leak where an unexpected error (e.g. an httpx failure during post-processing) could print a private feed URL/host into the public Actions log. Dev mode still re-raises the full error. Typer pretty-exceptions disabled so the guard actually sees the exception.
- **Classifier no longer wedges when a single batch exceeds the whole budget.** The token-budget pre-flight skipped any batch it couldn't afford — including the very first one. If `token_budget` was smaller than one batch's estimate (a misconfig, or one oversized item), every item was skipped, and since skipped items stay unclassified they skipped again next run: a permanent stall. The first batch of a run now always runs (forward progress), while the budget still gates every later batch.
- **GitHub-topic watermark no longer drops same-second repos.** `pushed_at` is second-granular and bursty; a strict `pushed <= watermark` skip silently dropped a genuinely-new repo pushed in the same second as the prior high-water repo. The cursor now also records the external ids seen *at* the watermark second, so same-second newcomers are emitted while already-seen repos are not re-emitted. (HN's `created_at_i` watermark keeps its strict `>` — the field is immutable and results are newest-first, so a same-second item is always in the same fetch as the watermark-setter; documented inline.)
- **arXiv error feeds no longer become items.** The export API answers a malformed query / transient fault with HTTP 200 and an Atom feed containing a single sentinel error entry (`arxiv.org/api/errors`). The adapter now filters those out, so the error notice is never stored, classified (burning tokens), or rendered as a paper.
