...
- **Daily-collect cron now actually persists state (was failing every run).** The first real run revealed a fatal bug: `git config user.name/email` was set on the *outer* main checkout, but the commit runs inside a freshly `git init`'d `_publish` clone with no identity — so `git commit` died with `Author identity unknown / Please tell me who you are`. Because the commit never succeeded, the `state` branch was never created, so the restore step also failed ("couldn't find ref `state`") on every run — one bug, both red steps. Identity is now set inside the `_publish` repo where the commit runs. The restore step was also rewritten as a graceful shallow `git fetch` (replacing `actions/checkout@v4 ref: state`, which shows red on a cold start even with `continue-on-error`) so it stays green when the branch doesn't exist yet.
